# Approver and lead approver are separate roles with a deliberate two-way permission-predicate split
...
Permission gates split two ways, deliberately (`server/utils/permissions.ts`, `server/utils/auth.ts`):
