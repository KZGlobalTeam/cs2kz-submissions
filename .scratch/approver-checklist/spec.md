...
21. As a lead approver, I want to never see the checklist section or its content anywhere, so that my approval decisions rest on votes and map facts, not approvers' private scratchpad.
22. As a submitting mapper, I want to never see any approver's checklist or note, so that review stays internal.
23. As an anonymous user, I want the checklist endpoints to reject me, so that private review data stays internal.
24. As a lead approver, I want the API to reject my checklist requests, so that the approver-only boundary holds server-side even against crafted requests.
25. As an approver, I want my checklist data removed when the submission is deleted, so that no orphaned private data lingers.
...
28. As a developer, I want the endpoints to read and upsert only the caller's own row keyed by submission and approver, so that concurrent saves from separate approvers never collide.
29. As a developer, I want checklist payloads validated against a shared body schema, so that malformed payloads are rejected with clear errors.
30. As a developer, I want unchecking every rule and clearing the note to persist as a real saved state, so that "reset to nothing" is not confused with "never saved".
31. As an approver, I want the section laid out beside the vote form on desktop and stacked on mobile, so that I can work both surfaces without horizontal scrolling.
...
- **Auth — strict plain-approver gate**: the user must have the `approver` role and not the `lead_approver` role. The shared approver guard deliberately admits lead approvers too, so a dedicated predicate is used at the API boundary.
...
- **Modules tested**: the shared checklist body schema (accept/reject behavior), and the role predicate (full truth table over role-set combinations: approver only, lead only, both, neither).
...
- **Governing decision**: ADR-0003 — private per-approver table, dedicated owned-resource endpoints, strict plain-approver gate, debounced auto-save with flush-on-vote-save/unmount, read-only surfacing after review.
