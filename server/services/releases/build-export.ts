 ...
 *  unknown (or other-game) release all live in the shared resolution; the
 *  pure shaping and the missing-filters refusal live in
 *  `toReleaseExportPayload`. */
export async function buildReleaseExport(releaseId: string, game: Game) {
  return toReleaseExportPayload(await resolveReleaseContents(releaseId, game))
}
